...
public class Client extends Player {
	...
	public void Messages() {
		EventManager.getSingleton().addEvent(new Event() {
			public void execute(EventContainer c) {
				...
				if (Message == 0) {
					sendMessage("[@red@SERVER NOTICE@bla@]@Vist our forums at www.DimeScape.com");
				} else if (Message == 1) {           
					sendMessage("[@red@SERVER NOTICE@bla@]Dont forget to ::vote Dailey");		
				} else if (Message == 2) { 
					sendMessage("[@red@SERVER NOTICE@bla@]Need Help? Contact a [MOD] or a [ADMIN]");
				} else if (Message == 3) {
					sendMessage("[@red@SERVER NOTICE@bla@]The Developer is Banter");
				} else if (Message == 4) {
					sendMessage("[@red@SERVER NOTICE@bla@]Donate to support the server!");
				}
			}
		}, 160000); //Milliseconds
	};
	...
	public void initialize() {
		...
		case 1:
			...
			if(bankpin != 0) { 
				sendMessage("@red@To Access Your bank for This session.");
				sendMessage("@red@Type ::enterpin <yourpin>");
			} else {
				sendMessage("@blu@You have no bankpin yet! Type ::newpin <yourpin> to set!");
			}
			pinset = 0;
		}
	}
	...
	public boolean skillCapeEquipped() {
				...
				for(int i=0;i<capes.length;i++) {
					if(playerEquipment[playerCape] == capes[i])
						return true;
				}
                return false;
        }
