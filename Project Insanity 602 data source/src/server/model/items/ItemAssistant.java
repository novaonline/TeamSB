...
public class ItemAssistant {
	...
	**/
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		} else if (WeaponName.startsWith("Staff of light")) {
	...
	**/
	...
	public void getRequirements(String itemName, int itemId) {
		...
		if(itemName.contains("statius's")) {
			c.attackLevelReq = 78;
		}
	...
	**/
	...
	public void addSpecialBar(int weapon) {
		switch(weapon) {
			...
			case 13758: // lime whip
			c.getPA().sendFrame171(0, 12323);
			specialAmount(weapon, c.specAmount, 12335);
			break;
			
			case 13760: // lava whip
			c.getPA().sendFrame171(0, 12323);
			specialAmount(weapon, c.specAmount, 12335);
			break;
	...
	**/
	...
	if ((wearID ==13790 || wearID ==1) && (c.playerRights < 4)) {
	c.sendMessage("You must be a donator to wear this.");
	return false;
	}
		...
		synchronized(c) {
			...
			if(c.playerItems[slot] == (wearID+1)) {				
				...
				if(itemType(wearID).equalsIgnoreCase("cape")) {
					...
					targetSlot=2;
				} else if(itemType(wearID).equalsIgnoreCase("Weapon")) {
					targetSlot=3;
					if (wearID == 619) {
     if (c.getItems().playerHasItem(619, 1)) {
     c.getItems().deleteItem(619, 1);
     int lottonumber = Misc.random(200);
     int winningnumber = Misc.random(200);
     int lottoreward = Misc.random(2147000000);
     if (lottonumber == winningnumber) {
     c.getItems().addItem(995, lottoreward);
     for (int i = 0; i < Config.MAX_PLAYERS; i++) {
     if (Server.playerHandler.players[i] != null) {
     Client c2 = (Client)Server.playerHandler.players[i];
     c2.sendMessage("<shad=15668759>[ANNOUNCEMENT] <img=2> Rune Wire: "+Misc.ucFirst(c.playerName)+" just won "+lottoreward+" GP from the lottery!");     }
     }
     } else {
     c.sendMessage("The winning number is "+winningnumber+". Your number is "+lottonumber+". The jackpot was "+lottoreward+" GP.");
     }
     }
     return false;
    }
